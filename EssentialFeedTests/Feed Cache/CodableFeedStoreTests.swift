    ...
    func test_retrieveAfterInsertingToEmptyCache_deliversInsertedValues() {
        ...
        sut.insert(feed, timestamp: timestamp) { insertionError in
            ...
            sut.retrieve { retrieveResult in
                ...
                default:
                    XCTFail("Expected found result with feed \(feed) qnd timestamp \(timestamp), got \(retrieveResult) instead")
                }
                exp.fulfill()
            }
        }
    ...
    func test_retrieve_hasNoSideEffectsOnNonEmptyCache() {
        ...
        sut.insert(feed, timestamp: timestamp) { insertionError in
            ...
            sut.retrieve { firstResult in
                sut.retrieve { secondResult in
                    switch (firstResult, secondResult) {
                    case let (.found(feed: firstFoundFeed, timestamp: firstFoundTimestamp), .found(feed: secondFoundFeed, timestamp: secondFoundTimestamp)):
